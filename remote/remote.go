...
type DockerRemote struct {
	...
	basicAuth       string // Password is used for basicAuth when WWW-Authenticate Header returns Basic
...
func (remote *DockerRemote) DoWithRetry(req *http.Request, numAttempts int, additionalScope ...string) (*http.Response, error) {
	...
	if remote.basicAuth != "" {
		req.Header.Set("Authorization", fmt.Sprintf("Basic %s", remote.basicAuth))
	}
	...
	if resp.StatusCode == http.StatusUnauthorized && numAttempts > 1 {
		...
		// We need either basic or bearer auth and try again...
